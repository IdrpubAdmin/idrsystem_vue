...
StyleCommon = (function(doc, global) {
	...
	obj.scrollToSmooth = function(_top){
		let headerHeight = doc.querySelector('.header-wrap') ? doc.querySelector('.header-wrap').offsetHeight : 0;
		window.scrollTo({
	...
	obj.animateScrollTo = function(_selector, _duration, _adjust) {
		_duration = 400; //이동시간
		_adjust = 60; //조정값
		let targetEle = document.querySelector(_selector);

		if (!targetEle){return;}

		// - Get current & target positions
		let scrollEle = document.documentElement || window.scrollingElement,
		...
		function animateScroll(_startY, _endY, _duration) {
			...
			let unitY = (targetY - currentY) / _duration;
			let startTime = new Date().getTime();
			let endTime = new Date().getTime() + _duration;
	...
	obj.updateDocTitle = function(txtTit) { // html title 변경하는 함수 (html에서 값 받아옴 (ex: 이랜드 신규브랜드 콸콸 구축 및 운영))
		if(txtTit) { 
			let tempTit = document.title; // 원래 title명
...
let Wrapper = Wrapper || {};
Wrapper = (function(doc, global) { //doc = #document, global = window
	// ELEMENTS INITIALIZE
	let obj = {},
		header = {
	...
	function initialize(){
		...
		// orientationchange 윈도우 가로/세로로 회전할때, 바디가 로드될때 orientationChangeHandler실행
	...
	function wrapperHandler(_evt){
		if(getBodyScrollTop() > beforeTop && getBodyScrollTop() > 0){ // UP GESTURE // 만약 스크롤한 값이 beforeTop(초기값 0)보다 크고 스크롤한 값이 0이상이라면
			// if(scroller.scrollTop > 0 && !doc.querySelector('#header').classList.contains('fixed')){ // ADD HEADER FIXED
			if(getBodyScrollTop() > 0 && !doc.querySelector('#header').classList.contains('fixed')){ // ADD HEADER FIXED // 만약 스크롤한 값이 0보다크고 헤더의 class가 fixed가 아니면 (스크롤 중이면)
			...
			if(getBodyScrollTop() == 0 && doc.querySelector('#header').classList.contains('fixed')) { // ON TOP SCROLL POSITION REMOVE HEADER FIXED //스크롤한 값이 0이고 헤더의 class가 fixed라면 (스크롤하다가 top으로 왔을때)
	...
	// (윈도우 가로/세로로 회전할때, 바디가 로드될때) - 가로모드일때 발동되는 이벤트 (class(.lands) 정해둔것이 없음)
	function orientationChangeHandler(){
		setTimeout(function(){
			if(body.offsetHeight < body.offsetWidth){ // LANDSCAPE 바디높이보가 바디너비가 클때 (가로모드)
	...
	//gnb_btn click function 햄버거버튼 클릭시 발동
	function gnbBtnHandler(_evt){
		let gnbWrap = doc.querySelector('.gnb_wrap'); // GNB모달
		if(gnbWrap.classList.contains('on')) { // GNB가 켜져있을때
...
WorksNav = (function(doc, global) {
	...
	let obj = {},
		body = doc.querySelector('body'),
		container = doc.querySelector('#container'),
		gnbD1 = doc.querySelector('.gnb_menu'), // gnb 모달
		lnbD1 = doc.querySelector('.lnb_d1_wrap'); // lnb1 (ul)

	function vdrawLnbNa(){ //handlebars draw drawLnbNa() 핸들바 문법
		//reference
		let hndBrWrap=$(".lnb_d1_wrap"); // lnb1 (ul)
		let hndBrSource=$("#drawLnbNavTmp").html(); // 핸들바 템플릿 가져오기
		let hndBrCompileTemp=Handlebars.compile(hndBrSource); // 핸들바 템플릿 컴파일
		hndBrWrap.empty().html(hndBrCompileTemp(lnbInfoData)); // 바인딩해서 html 생성
	}
	
	// PRIVATE FUNCTIONS
	function initialize(){ // 위에 이니셜라이즈 함수 실행되면
	...
	obj.checkCurrentLnbNav = function(gnb,lnb1,lnb2) { // gnb - 보고있는 페이지 들의 gnb위치 (ex: service 1 works 2 contact 3), lnb1 - lnb위치(ex: 2021 1 2020 2 ...) , lnb2 - lnb2위치
		setTimeout(function(){
			...
			Array.prototype.slice.call(gnbD1.querySelectorAll('li')).forEach(function(_el,_idx){
				let tempTxt1 = _el.querySelector('a').getAttribute('data-menu-id'); // tempTxt1 = gnb메뉴의 a의 data-menu-id값
				if(tempTxt1 === gnb){ // data-menu-id값이 보고있는 페이지 들의 gnb위치와 일치할때 (현재 위치한 페이지와 gnb메뉴의 값이 일치할때)
			...
			Array.prototype.slice.call(lnbD1.querySelectorAll('.d1_li')).forEach(function(_el1,_idx1){
				...
				if(tempTxt1 === lnb1){ // data-menu-id값이 보고있는 페이지 들의 lnb위치와 일치할때 (현재 위치한 페이지와 lnb메뉴의 값이 일치할때)
					_el1.classList.add('on');
				}
				let lnbD2 = _el1.querySelector('.lnb_d2_wrap'); // lnb메뉴의 lnb(ex: works 2021의 "1 2 3 4")
				if(lnbD2) {
					lnbD2.querySelectorAll('.d2_li').forEach(function(_el2,_idx2) { // lnb2 들
						let tempTxt2 = _el2.querySelector('a').getAttribute('data-menu-id');
						if(tempTxt2 === lnb2 && _el1.classList.contains('on')){ // data-menu-id값이 보고있는 페이지와 같고 lnb값이 on일때 (보고있지 않은 다른연도에 on 붙는걸 방지)
